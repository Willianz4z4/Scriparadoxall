-- ==========================================================
-- SISTEMA DE KEY - PLATOBOOST (BLINDAGEM DUPLA PARA DELTA)
-- ==========================================================
...
-- ==========================================================
-- 1. CRIANDO A INTERFACE GRÁFICA (UI)
-- ==========================================================
...
-- ==========================================================
-- 3. MOTOR DO PLATOBOOST E CRIPTOGRAFIA
-- ==========================================================
...
local function SafeRequest(options)
    ...
    -- Retorna erro controlado se o executor falhar
    return { StatusCode = 0, Body = '{"success":false,"message":"Erro interno no executor."}' }
end
    ...
    if cachedTime + (10*60) < fOsTime() then
        ...
        if response.StatusCode == 200 then
            ...
            if decodeOk and decoded.success == true then
                ...
                onMessage(decodeOk and decoded.message or "Dados inválidos recebidos."); return false, "Erro";
            end
...
local verifyKey = function(key)
    ...
    -- 🚨 PLANO B: Se a função principal falhar, usa game:HttpGet (Nativo do Roblox)
    if response.StatusCode == 0 or response.StatusCode == nil then
        local ok, getBody = pcall(function() return game:HttpGet(endpoint) end)
        if ok and type(getBody) == "string" and string.find(getBody, "success") then
            response = { StatusCode = 200, Body = getBody }
        end
    end
        ...
        if decodeOk and decoded.success == true then
            if decoded.data.valid == true then
                if useNonce then
                    ...
                    else onMessage("Falha de segurança."); return false; end
                else return true; end
            else
                onMessage("A Key inserida é inválida."); return false;
            end
        else onMessage(decodeOk and decoded.message or "Servidor falhou ao responder."); return false; end
    elseif response.StatusCode == 429 then
        onMessage("Limite excedido. Aguarde 20 segundos."); return false;
    else onMessage("Falha ao se conectar com o Platoboost."); return false; end
end

-- ==========================================================
-- 4. CONECTANDO OS BOTÕES (ANTI-CONGELAMENTO)
-- ==========================================================
    ...
    task.spawn(function()
        ...
        if success then
            if fSetClipboard then
                ...
                GetKeyBtn.Text = "Erro"
                StatusText.Text = "Seu executor não suporta cópia."
            end
        else
            GetKeyBtn.Text = "Erro"
        end
...
VerifyBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if isValid then
            ...
            StatusText.Text = "Key válida! Iniciando..."
            StatusText.TextColor3 = Color3.fromRGB(0, 255, 0)
            task.wait(1)
            StartMainScript()
        else
