-- ==========================================================
-- SISTEMA DE KEY - PLATOBOOST (COM RAIO-X DE ERROS)
-- ==========================================================
...
-- ==========================================================
-- SCRIPT PRINCIPAL E PLATOBOOST
-- ==========================================================
local function StartMainScript()
    print("✅ [PARADOX] Key validada!")
    if ScreenGui then ScreenGui:Destroy() end
    -- AQUI ELE PUXA O SEU JOGO (O ARQUIVO MAIN.LUA DEVE ESTAR LIMPO!)
...
-- Função Raio-X de Rede (Mostra o erro exato)
local function SafeRequest(options)
    ...
    -- Se o Delta estiver bloqueando requisições via loadstring:
    return { StatusCode = 500, Body = '{"success":false,"message":"O Delta bloqueou a internet do Loadstring."}' }
end
    ...
    if cachedTime + (10*60) < fOsTime() then
        ...
        if response.StatusCode == 200 then
            ...
            if decodeOk and decoded.success == true then
                ...
                onMessage(decodeOk and decoded.message or "Erro de dados do servidor."); return false;
            end
        elseif response.StatusCode == 429 then
            onMessage("Aguarde 20 segundos antes de tentar."); return false;
        else
            -- A MÁGICA ESTÁ AQUI: Vai escrever na tela exatamente o que deu errado!
            local erroReal = string.sub(tostring(response.Body), 1, 40)
            onMessage("ERRO HTTP " .. tostring(response.StatusCode) .. " | " .. erroReal); 
            return false;
        end
    else
        ...
        if decodeOk and decoded.success == true then
            if decoded.data.valid == true then
                if useNonce then
                    ...
                    else onMessage("Falha de segurança do Platoboost."); return false; end
...
task.spawn(function()
    if isfile and isfile(KeyFileName) then
        ...
        if verifyKey(savedKey) then StartMainScript() else if delfile then pcall(function() delfile(KeyFileName) end) end end
    end
end)
