...
-- ==========================================================
-- 3. MOTOR DO PLATOBOOST (Criptografia e Conexão)
-- ==========================================================
...
-- Conecta as mensagens de erro do Platoboost direto na nossa interface
...
function cacheLink()
    if cachedTime + (10*60) < fOsTime() then
        ...
        elseif response.StatusCode == 429 then
            local msg = "Limite de requisições. Aguarde 20 segundos.";
            onMessage(msg);
            return false, msg;
        end
        local msg = "Falha ao gerar o link do Platoboost.";
        onMessage(msg);
        return false, msg;
    else
        return true, cachedLink;
...
local redeemKey = function(key)
    ...
    if response.StatusCode == 200 then
        ...
        if decoded.success == true then
            ...
            else onMessage("A Key é inválida."); return false; end
        else
...
local verifyKey = function(key)
    if requestSending == true then onMessage("Aguarde, enviando requisição..."); return false; else requestSending = true; end
    local nonce = generateNonce();
    ...
    if response.StatusCode == 200 then
        ...
        if decoded.success == true then
            if decoded.data.valid == true then
                ...
                else onMessage("A Key inserida é inválida."); return false; end
            end
        else onMessage(decoded.message); return false; end
    elseif response.StatusCode == 429 then
        onMessage("Limite excedido. Aguarde 20 segundos."); return false;
    else onMessage("Erro do servidor Platoboost."); return false; end
end

-- ==========================================================
-- 4. CONECTANDO OS BOTÕES DA UI AO PLATOBOOST
-- ==========================================================
...
VerifyBtn.MouseButton1Click:Connect(function()
    ...
    if isValid then
        ...
        VerifyBtn.Text = "Erro!"
        -- O texto do erro é gerado automaticamente pelo motor do Platoboost
        task.wait(2.5)
